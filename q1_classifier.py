...
class SoftmaxModel(Model):
  ...
  def add_training_op(self, loss):
    ...
    """
    ### YOUR CODE HERE
    opt=DoubleGDOptimizer(learning_rate=tf.constant(Config.lr,tf.float64))
  ...
  def add_model(self, input_data):
    ...
    W=tf.to_float(tf.Variable(np.zeros(n_features*n_classes).reshape(n_features,n_classes),name="W"))
    b=tf.to_float(tf.Variable(np.zeros(n_classes),name="b"))
    y=tf.matmul(input_data,W)
    x=tf.add(y,b)
    xExp=tf.exp(x)
    sumExp=tf.reshape(tf.reduce_sum(xExp,1),[Config.batch_size,1])
    out=tf.div(xExp,sumExp)
    sess.close()
    ### END YOUR CODE
    return out
  ...
  def add_loss_op(self, pred):
    ...
    prodEl=tf.mul(tf.to_float(y),tf.to_float(pred))
    out=tf.reduce_sum(tf.log(tf.reduce_sum(prodEl,1)))
    loss=-out
    ### END YOUR CODE
    return loss
