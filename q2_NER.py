...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    """
    ### YOUR CODE HERE
    xavInit=xavier_weight_init()
    hidden_1=tf.to_float(tf.Variable(np.random.uniform(size=self.config.hidden_size),name="h1"))    
    output=tf.to_float(tf.Variable(np.random.uniform(size=(self.config.batch_size,self.config.label_size)),name="output"))
    print "output def",output.dtype
    with tf.variable_scope("Layer"):
      inp=xavInit((self.config.window_size*self.config.embed_size,self.config.hidden_size))
      W=tf.to_float(tf.Variable(inp,name="W"))
      bias=xavInit((self.config.hidden_size,))
      b1=tf.expand_dims(tf.to_float(tf.Variable(bias,name="b1")),1)
      print window.get_shape()
      print W.get_shape()
      h1=tf.add(tf.transpose(tf.matmul(window,W)),b1)
      hidden_1=tf.tanh(h1)
    with tf.variable_scope("Softmax"):
      inp=xavInit((self.config.hidden_size,self.config.label_size))
      U=tf.to_float(tf.Variable(inp,name="U"))
      bias=xavInit((self.config.label_size))
      b2=tf.to_float(tf.Variable(bias,name="b2"))
      print hidden_1.dtype,"\n",U.dtype,"\n",b2.dtype
      out=tf.add((tf.transpose(tf.matmul(tf.transpose(hidden_1),U))),tf.to_float(tf.expand_dims(b2,1)))
      
      output=tf.to_float(tf.transpose(out))
      print output.dtype
    ### END YOUR CODE
    return output
  ...
  def add_loss_op(self, y):
    ...
    loss=tf.to_float(tf.reduce_mean(loss))
    ### END YOUR CODE
    return loss
