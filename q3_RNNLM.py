...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      allSteps=tf.split(1,self.config.num_steps,inputs)
      inputs=[tf.squeeze(i,[1]) for i in allSteps]
      ### END YOUR CODE
      return inputs
  ...
  def add_loss_op(self, output):
    ...
    crossEntropy=sequence_loss([output],[tf.reshape(self.labels_placeholder,[-1])],one,len(self.vocab))
    tf.add_to_collection('ce_loss',crossEntropy)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ### YOUR CODE HERE
    feedDict={model.input_placeholder:[tokens[-1:]],model.initial_state:state,model.dropout_placeholder:1}
    state,y_pred=session.run([model.final_state,model.predictions[-1]],feed_dict=feedDict)
    ### END YOUR CODE
